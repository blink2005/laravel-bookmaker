        ...
        <section class="pl-[20px] w-fit mt-[126px]">
            @if (count($pari) > 0)
                @foreach ($pari as $value)
                    @if (! str_contains($value->event_result, '_hide'))
                        <p class="text-[24px] font-[700]">Событие: {{ $value->event_name }}</p>
                        <p>Ставка на: {{ $value->event_team }} || {{ $value->event_result }}</p>
                        <p>Сумма: {{ $value->sum_bet }} RUB</p>
                        <p>Коэффициент: {{ $value->coefficient }}x</p>
                        <p>Статус: {{ $value->status }}</p>
                        <br>
                    @else
                        <p class="text-[24px] font-[700]">Событие: {{ $value->event_name }}</p>
                        <p>Ставка на: {{ $value->event_team }}</p>
                        <p>Сумма: {{ $value->sum_bet }} RUB</p>
                        <p>Коэффициент: {{ $value->coefficient }}x</p>
                        <p>Статус: {{ $value->status }}</p>
                        <br>
                ...
                @endforeach
            @else
                <p class="text-[24px] font-[700]">У вас нет активных пари</p>
