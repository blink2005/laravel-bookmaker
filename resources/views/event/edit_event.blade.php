...
@if (! $auth_status)
    ...
    <div class="w-[500px] mt-[126px] mb-[20px] ml-[20px] mr-[20px] bg-[#141414] shadow-md px-8 pt-6 pb-8" method="POST">
        <form action="/editevent/save" method="POST">
            @csrf
            <p class="w-fit text-[24px] font-[700]">{{ $event->event_name }} | Parameters</p>
            <textarea class="mt-4 hadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="15" name="parameters" placeholder="Параметры ставок JSON" required>{{ $event->parameters }}</textarea>
