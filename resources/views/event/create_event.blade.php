...
@if (! $auth_status)
    ...
    <form class="w-[400px] mt-[126px] mb-[20px] ml-[20px] mr-[20px] bg-[#141414] shadow-md px-8 pt-6 pb-8" method="POST">
        @csrf
        @if ($errors->any())
            @foreach ($errors->all() as $error)
                <p class="text-red-700 text-center text-[24px] mb-4">{{ $error }}</p>
            @endforeach
        @else
            <p class="text-center text-[24px] mb-4">Создание события</p>
        @endif

        <div class="mb-4">
            <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                name="category" type="text" placeholder="Категория" required>
        </div>
        <div class="mb-4">
            <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                name="name" type="text" placeholder="Название" required>
        </div>
        <div class="mb-4">
            <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                name="date" type="text" placeholder="Дата начала" required>
        </div>
        <div class="mb-4">
            <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                name="time" type="text" placeholder="Время начала" required>
        </div>
        <div class="mb-4">
        <textarea class="hadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="5" name="parameters" placeholder="Параметры ставок JSON" required></textarea>
        </div>
        <input
