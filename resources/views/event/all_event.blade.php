...
@if (! $auth_status)
    ...
    <main>
        <section class="pl-[20px] w-fit mt-[126px]">
            <p class="w-fit text-[24px] font-[700]">Выберите событие</p>
            @foreach ($event as $value)
                <a href="{{ route('edit_event_id', ['id' => $value->id]) }}" class="w-fit block text-[16px] font-[700] text-cyan-500">-> {{ $value->event_name }}</a>
            @endforeach
