...
@if (! $auth_status)
    ...
    <main>
        <section class="pl-[20px] w-fit mt-[126px]">
            @if (count($category) > 0)
                <p class="w-fit text-[24px] font-[700]">Выберите категорию</p>
                
                @foreach ($category as $value)
                    <a href="{{ route('category_name', ['name' => $value->name_category]) }}" class="w-fit block text-[16px] font-[700] text-cyan-500">-> {{ $value->name_category }}</a>
                @endforeach
            @else
                <p class="w-fit text-[24px] font-[700]">Отсутствуют активные категории</p>
