        ...
        <form class="w-[400px] mt-[126px] mb-[20px] ml-[20px] mr-[20px] bg-[#141414] shadow-md px-8 pt-6 pb-8" method="POST">
            ...
            @if ($errors->any())
                @foreach ($errors->all() as $error)
                    <p class="text-red-700 break-words text-[24px] mb-4">{{ $error }}</p>
                @endforeach
            @else
                <p class="break-words text-[24px] mb-4">Введите сумму вывода</p>
            @endif

            <div class="mb-4">
                <input
                    class="shadow appearance-none border rounded w-[110px] py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    name="sum" type="number" placeholder="Сумма" required>
            </div>

            <div class="mb-4">
                <input
                    class="shadow appearance-none border rounded w-[110px] py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    name="description" type="text" placeholder="Реквизиты" required>
            </div>


            <div class="mb-4">
                <input
                    class="text-right bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded focus:outline-none focus:shadow-outline"
                    type="submit" value="Вывести">
                </input>
            </div>
        </form>
    </main>
