...
@if (! $auth_status)
    ...
    @endif
    <main>
        <div class="w-[400px] mt-[126px] mb-[20px] ml-[20px] mr-[20px] bg-[#141414] shadow-md px-8 pt-6 pb-8">
            <p class="break-words text-[24px] mb-4">id пользователя: {{ $withdrawal->id_user }}</p>
            <p class="break-words text-[16px] mb-4">Сумма вывода: {{ $withdrawal->sum_withdrawals }} RUB</p>
            <p class="break-words text-[16px] mb-4">Реквизиты: {{ $withdrawal->description }}</p>

            <div class="flex">
                <form action="payments/accept" method="POST">
                    @csrf
                    <input type="hidden" name="withdrawal_id" value="{{ $withdrawal->id }}" autocomplete="off">
                    <input
                    class="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                    type="submit" value="Приянть">
                    </input>
                </form>

                <form action="payments/cancel" method="POST">
                    @csrf
                    <input type="hidden" name="withdrawal_id" value="{{ $withdrawal->id }}" autocomplete="off">
                    <input
                    class="ml-2 bg-red-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                    type="submit" value="Отказать">
                    </input>
                </form>
            </div>
